...
define('APP_TEXTDOMAIN', 'sepandrah');
...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_enqueue_script(
        'lenis',
        'https://unpkg.com/lenis@1.3.13/dist/lenis.min.js',
        [],
        '1.3.13',
        true
    );
    ...
    // =========================
    // Defer strategy (WP 6.3+)
    // =========================
    // if ( function_exists('wp_script_add_data') ) {
    //     foreach (['gsap','gsap-scrolltrigger','gsap-scrollto','swiper','lenis'] as $h) {
...
add_filter('wp_resource_hints', function ($hints, $relation_type) {
    ...
    if ($relation_type === 'dns-prefetch') {
        ...
        foreach ($hosts as $host) {
            $hints[] = ['href' => $host, 'crossorigin' => true];
        }
    }
    return $hints;
}, 10, 2);
